...
int main (int argc, char *argv[])
{
  ...
  Ptr<EnergySourceContainer> esCont0 = CreateObject<EnergySourceContainer> ();
  es0->SetNode (n0);
  es0->AppendDeviceEnergyModel (em0);
  em0->SetEnergySource (es0);
  em0->AttachPhy (dev0->GetPhy());
  esCont0->Add (es0);
  n0->AggregateObject (esCont0);

  Ptr<EnergySourceContainer> esCont1 = CreateObject<EnergySourceContainer> ();
  es1->SetNode (n1);
  es1->AppendDeviceEnergyModel (em1);
  em1->SetEnergySource (es1);
  em1->AttachPhy (dev1->GetPhy());
  esCont1->Add (es1);
  n1->AggregateObject (esCont1);
  ...
  Simulator::ScheduleWithContext (1, Seconds (1.0),&LrWpanMac::McpsDataRequest,dev0->GetMac (), params, p0); // Request to transfer a MSDU
